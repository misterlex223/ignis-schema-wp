 ...
 * ACF Field Generator
 *
 * Converts schema field definitions to ACF field group arrays
...
class ACFFieldGenerator {
    ...
    /**
     * Generate ACF field group from schema
     *
    ...
    public static function generateFieldGroup($schema) {
        ...
        if (!empty($schema['fields']) && is_array($schema['fields'])) {
            foreach ($schema['fields'] as $field_key => $field_config) {
                $acf_field = self::generateField($field_key, $field_config, $post_type);
                if ($acf_field) {
